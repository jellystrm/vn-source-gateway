...
def _season_ep_count(tmdb_id: int | None, season: int, settings: Any) -> int | None:
    """Return episode count for a season from TMDB, or None if unavailable."""
...
def _resolve_ep_fresh(
    ...
    """Resolve one episode using a fresh source instance (safe for parallel use)."""
    from deceptarr.sources import build_sources
    from deceptarr.domain.models import EpisodeWanted
...
def build_handler() -> type[BaseHTTPRequestHandler]:
    class Handler(BaseHTTPRequestHandler):
        ...
        def _handle_source_test(self) -> None:
            ...
            # all-episodes mode: season given but episode omitted
